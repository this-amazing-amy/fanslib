...
import {
  ...
  refreshRedgifsURL,
} from "./operations/redgifs/refresh-url";
import {
  FindSubredditPostingTimesRequestBodySchema,
  findSubredditPostingTimes,
} from "./operations/subreddit/find-posting-times";

export const postponeRoutes = new Hono()
  .basePath("/api/postpone")
  .post(
    "/draft-bluesky",
    zValidator("json", DraftBlueskyPostRequestBodySchema, validationError),
    async (c) => {
  ...
  .post(
    "/find-redgifs-url",
    zValidator("json", FindRedgifsURLRequestBodySchema, validationError),
    async (c) => {
      const body = c.req.valid("json");
  ...
  .post(
    "/refresh-redgifs-url",
    zValidator("json", RefreshRedgifsURLRequestBodySchema, validationError),
    async (c) => {
      const body = c.req.valid("json");
  ...
  .post(
    "/find-subreddit-posting-times",
    zValidator("json", FindSubredditPostingTimesRequestBodySchema, validationError),
    async (c) => {
      const body = c.req.valid("json");
