...
import {
  ...
  createFilterPreset,
} from "./operations/filter-preset/create";
...
import {
  ...
  updateFilterPreset,
} from "./operations/filter-preset/update";
...
export const filterPresetsRoutes = new Hono()
  ...
  .post(
    "/",
    zValidator("json", CreateFilterPresetRequestBodySchema, validationError),
    async (c) => {
  ...
  .patch(
    "/by-id/:id",
    zValidator("json", UpdateFilterPresetRequestBodySchema, validationError),
    async (c) => {
      const id = c.req.param("id");
      const body = c.req.valid("json");
