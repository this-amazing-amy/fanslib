...
import {
  ...
  updateHashtagStats,
} from "./operations/hashtag-stats/update";
...
export const hashtagsRoutes = new Hono()
  ...
  .get(
    "/by-ids",
    zValidator("query", FetchHashtagsByIdsQuerySchema, validationError),
    async (c) => {
      const query = c.req.valid("query");
      const ids = query.ids ? JSON.parse(query.ids) : [];
      const numberIds = ids.map((id: unknown) => (typeof id === "string" ? parseInt(id, 10) : id));
      const result = await fetchHashtagsByIds(
        numberIds.filter((id: number) => !isNaN(id)) as number[],
      );
      return c.json(result);
    },
  )
  ...
  .post(
    "/",
    zValidator("json", FindOrCreateHashtagRequestBodySchema, validationError),
    async (c) => {
  ...
  .post(
    "/by-ids",
    zValidator("json", FindOrCreateHashtagsByIdsRequestBodySchema, validationError),
    async (c) => {
      const body = c.req.valid("json");
  ...
  .post(
    "/by-id/:id/stats",
    zValidator("json", UpdateHashtagStatsRequestBodySchema, validationError),
    async (c) => {
      const id = parseInt(c.req.param("id"));
      const body = c.req.valid("json");
