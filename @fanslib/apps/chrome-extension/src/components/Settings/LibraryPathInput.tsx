...
const VerifyButton = ({ status, verifiedFolderName, pathMismatch, onClick }: VerifyButtonProps) => {
  ...
  const buttonContent = isChecking ? (
    ...
    <>
      ...
      {pathMismatch && <span className="text-warning text-xs ml-1">(path mismatch)</span>}
    </>
  ) : (
    <>
...
export const LibraryPathInput = ({ libraryPath, apiUrl, onChange }: LibraryPathInputProps) => {
  const { status, errorMessage, verifiedFolderName, verify } = useLibraryVerification();
  ...
  return (
    <div>
      <label className="block text-sm font-medium mb-2">Library Folder Path</label>
