...
import { clearActivityLog, getActivityLog, type ActivityLogEntry } from "../../lib/activity-log";
    ...
    return (
      <div className="px-3 pt-3 pb-4">
        <div className="flex items-center justify-between mb-3">
          ...
          {entries.length > 0 && (
            <button onClick={handleClear} className="btn btn-ghost btn-xs">
              Clear
            </button>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="text-sm text-base-content/50 text-center py-8">No activity yet</div>
        ) : (
    ...
    <div className="border-t border-base-300 bg-base-200/50 shrink-0">
      ...
      >
        ...
        {latestEntry && !expanded && (
          <span className="flex items-center gap-1.5 flex-1 min-w-0">
            <span
              className={`w-1.5 h-1.5 rounded-full shrink-0 ${typeStyles[latestEntry.type].dot}`}
            />
            <span className="text-[11px] text-base-content/70 truncate">{latestEntry.message}</span>
      ...
      {expanded && (
        <div className="border-t border-base-300">
          ...
          <div ref={scrollRef} className="max-h-40 overflow-y-auto px-2 pb-2">
            {loading ? (
