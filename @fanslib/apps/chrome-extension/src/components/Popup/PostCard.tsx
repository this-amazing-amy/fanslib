...
import { Calendar, Check, Copy, ExternalLink, Image as ImageIcon, Video } from "lucide-react";
...
export const PostCard = ({
  ...
  const [captionCopyState, setCaptionCopyState] = useState<"idle" | "copied">("idle");
    ...
    <div className="rounded-xl p-3 bg-base-100 relative">
      ...
      {imageErrors.size > 0 && (
        <div className="mt-3 p-2 bg-error/10 border border-error/20 rounded text-xs">
          <button onClick={() => setShowDebug(!showDebug)} className="text-error font-medium mb-1">
            {showDebug ? "▼" : "▶"} Debug: {imageErrors.size} thumbnail(s) failed to load
          </button>
          {showDebug && (
