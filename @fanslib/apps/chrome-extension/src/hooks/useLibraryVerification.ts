...
const fetchSampleMediaPath = async (apiUrl: string): Promise<string> => {
  ...
  if (!data?.items?.length) {
    throw new Error("No media found in library. Please ensure your library has been scanned.");
  }
...
const buildUpdatedPath = (currentPath: string, folderName: string): string | null => {
  if (!currentPath) {
    return folderName;
  }
...
export const useLibraryVerification = (): UseLibraryVerificationResult => {
  ...
  const [verifiedFolderName, setVerifiedFolderName] = useState<string | null>(null);

  const verify = async (apiUrl: string, currentPath: string): Promise<string | null> => {
    ...
    try {
      ...
      const result: VerificationResult = await verifyDirectoryAccess(relativePath);
