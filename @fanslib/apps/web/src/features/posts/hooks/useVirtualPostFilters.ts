...
export const useVirtualPostFilters = ({
  channelId,
  scheduleId,
}: UseVirtualPostFiltersParams): MediaFilter => {
  const { data: schedule } = useContentScheduleQuery(scheduleId ?? "");
  const { data: channel } = useChannelQuery({ id: channelId });

  return useMemo(() => {
    const scheduleFilters =
      schedule && !("error" in schedule) && schedule.mediaFilters
        ? (parseMediaFilters(schedule.mediaFilters) ?? [])
        : [];
    const channelFilters =
      channel && !("error" in channel) && channel.eligibleMediaFilter
