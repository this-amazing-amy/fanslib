      ...
      options={[
        {
          ...
          icon: (
            <ChannelTypeIcon
              typeId={post.channel.typeId as ChannelTypeId}
              className="h-4 w-4"
              color="oklch(0.45 0.1 270)"
            />
          ),
      ...
      <div className="grid grid-cols-7 gap-4 max-h-[600px] overflow-y-auto py-2">
        {days.map((day) => {
          ...
          return (
            <div
              key={day.toISOString()}
              className={`flex flex-col rounded-lg p-2 ${isCurrentPostDay ? "bg-base-200" : ""}`}
            >
              {/* Day header */}
              <div className="flex items-center gap-1.5 mb-2 flex-shrink-0">
                <span
                  className={`font-medium text-sm ${isCurrentPostDay ? "bg-primary text-primary-content rounded-full w-6 h-6 flex items-center justify-center" : "w-6 h-6 flex items-center justify-center"}`}
                >
                  {format(day, "d")}
                </span>
                  ...
                  return (
                    <PostCalendarPostView
                      ...
                      wrapper={
                        isCurrentPost
                          ? (children: ReactNode) => (
                              <div className="ring-3 ring-primary rounded-xl [&>*]:border-0">
                                {children}
                              </div>
                            )
                          : (children: ReactNode) => (
