        ...
        {linkedPosts.map((linked, index) => {
          ...
          const post =
            postQuery.data && typeof postQuery.data === "object" && "id" in postQuery.data
              ? postQuery.data
              : null;
          const channel = post?.channel as
            | { name: string; typeId: string; type?: { id: string } }
            | undefined;
          const schedule = post?.schedule as
            | { name: string; emoji: string | null; color: string | null }
            | null
            | undefined;
          const subreddit = post?.subreddit as { name: string } | null | undefined;
          const channelName =
            channel?.typeId === "reddit" && subreddit?.name
              ? `r/${subreddit.name}`
              : (channel?.name ?? linked.channelName);
