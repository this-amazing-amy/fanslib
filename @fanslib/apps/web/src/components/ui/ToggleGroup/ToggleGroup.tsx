...
const ToggleGroupItem = ({ option, state, variant, size, className }: ToggleGroupItemProps) => {
  ...
  return (
    ...
    >
      ...
      {option.icon && <span className={cn(option.label ? "mr-2" : undefined)}>{option.icon}</span>}
      {option.label}
    </label>
  );
};
