...
export const SelectValue = ({ placeholder, className }: SelectValueProps) => (
  ...
  >
    {({ selectedText }) => (
      <span data-placeholder={!selectedText ? "" : undefined}>{selectedText || placeholder}</span>
    )}
  </AriaSelectValue>
);
...
export const SelectGroup = ({ children, label, className }: SelectGroupProps) => (
  <div className={cn("py-1", className)}>
    {label && (
      <div className="px-4 py-2 text-xs font-semibold text-base-content/70 uppercase">{label}</div>
    )}
    {children}
  </div>
);
...
export const SelectSeparator = ({ className }: { className?: string }) => (
  <div className={cn("my-1 h-px bg-base-content/20", className)} />
);
...
export const SelectLabel = ({ children, className }: SelectLabelProps) => (
  <Label className={cn("text-sm font-medium mb-1 block", className)}>{children}</Label>
);
